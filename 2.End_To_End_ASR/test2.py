...
CHECKPOINT_SAVING_PATH="./checkpoints/lstm.ckpt-1"
...
def test(tfrecords_file_list):
    ...
    with tf.Session(config=config) as sess:
        #restore
        saver.restore(sess=sess,save_path=CHECKPOINT_SAVING_PATH)
        #eval
        for epoch in range(1, timit_parameter.MAX_EPOCH + 1):
            ...
            for i in range(0, (timit_parameter.TRAIN_SIZE // timit_parameter.BATCH_SIZE)):
                ...
                loss_,decoded_dense_,error_=sess.run(
                    fetches=[loss,decoded_dense,error],
